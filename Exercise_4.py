...
def mergeSort(arr):
    if len(arr)>1:
        ...
        mergeSort(right)
        i=j=k =0
        while i<len(left) and j<len(right):   #compares left half and right half of the array and store it in resultant sorted array
            if left[i]< right[j]:
                arr[k] = left[i]
                i+=1
        ...
        while i<len(left):                  #to make sure the order of the elements added to the resultant list is right.
            arr[k] = left[i]
            i+=1
            k+=1
